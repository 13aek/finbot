...
def _get_random_by_category(category: str, limit: int) -> List[FinProduct]:
    ...
    """
    return list(
        FinProduct.objects.filter(category=category)
        .order_by("?")[:limit]
    )
...
def get_main_recommendations_for_user(user) -> Tuple[List[FinProduct], int]:
    ...
    bookmarked_qs = (
        FinProduct.objects
        .filter(bookmark_lists__user=user)
        .distinct()
    )
    ...
    if bookmark_count < 10:
        ...
        random_qs = (
            FinProduct.objects
            .exclude(bookmark_lists__user=user)  # 내가 이미 북마크한건 제외
            .order_by("?")[:needed]
        )
    ...
    popular_qs = (
        bookmarked_qs
        .annotate(bookmark_total=Count("bookmark_lists"))
