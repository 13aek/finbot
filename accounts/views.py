...
def extract_period(text):
    ...
    return int(match.group(1)) if match else 12  # 기본값 12개월
...
def update(request):
    ...
    for fav in favorites:
        ...
        rate_percent = extract_rate(p.maturity_interest)
        rate = rate_percent / 100

        months = extract_period(p.product_name)
        days = months * 30

        expected_profit = int(principal * rate * (days / 365))
        total_amount = principal + expected_profit
